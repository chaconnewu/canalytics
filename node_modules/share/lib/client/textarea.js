...
replaceText = function(newText, transformCursor, elem) {
...
window.sharejs.extendDoc('attach_textarea', function(elem) {
  var doc, event, prevvalue, _i, _len, _ref, _results;
  doc = this;
  elem.value = this.getText();
  prevvalue = elem.value;
  this.insertHandler = function(pos, text) {
    var transformCursor;
    ...
    return replaceText(prevvalue.slice(0, pos) + text + prevvalue.slice(pos), transformCursor, elem);
  };
  this.deleteHandler = function(pos, text) {
    var transformCursor;
    transformCursor = function(cursor) {
      if (pos < cursor) {
    ...
    prevvalue = elem.value.replace(/\r\n/g, '\n');
    return replaceText(prevvalue.slice(0, pos) + prevvalue.slice(pos + text.length), transformCursor, elem);
  };
  this.on('insert', this.insertHandler);
  this.on('delete', this.deletehandler);
  this.genOp = function(event) {
    var onNextTick;
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (elem.addEventListener) {
      _results.push(elem.addEventListener(event, this.genOp, false));
    } else {
      _results.push(elem.attachEvent('on' + event, this.genOp));
    }
  }
  return _results;
});

window.sharejs.extendDoc('detach_textarea', function(elem) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    event = _ref[_i];
    elem.removeEventListener(event, this.genOp, false);
  }
  this.removeListener('insert', this.insertHandler);
  return this.removeListener('delete', this.deleteHandler);
});
