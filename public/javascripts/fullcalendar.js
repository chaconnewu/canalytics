...
var fc = $.fullCalendar = { version: "1.5.3.4" };
	...
	function changeView(newViewName) {
		if (!currentView || newViewName != currentView.name) {
			...
			if (currentView) {
				...
				try{
				viewInstances[newViewName] = new fcViews[newViewName](
...
function vpadding(element) {
	try{
	var a = parseFloat($.css(element[0], 'paddingTop', true));
	var b= parseFloat($.css(element[0], 'paddingBottom', true));
	var c= (a || 0) + (b || 0);
}catch(err){
	console.log(err);
}
